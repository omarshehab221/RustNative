...
tier except the other backends, which come later). Done so far: **Milestone 47 — state, resilience, and data** (scoped stores read by slice, error boundaries with supervision, a query cache with offline mutations, forms, migrations, and WinHTTP with certificate pinning; `docs/data.md`), **Milestone 46
