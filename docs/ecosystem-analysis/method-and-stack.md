...
| L3 | Layout survives text scaling, translation growth, and right-to-left mirroring; text is shaped and measured by the host's own stack; update cost is proportional to what changed; every authoring surface offered reaches the whole API, with equality proven rather than claimed; style resolves per node against tokens, and every style spelling offered reaches the same properties, with what a host cannot realize answered at build time |
